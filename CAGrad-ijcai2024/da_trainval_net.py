...
if __name__ == '__main__':
  ...
  output_dir = args.save_dir + "/" + args.net + "/" + args.dataset+"/" +"20211019"
  ...
  if args.resume:
    load_name = os.path.join(output_dir,
       '20211019_city2fogcity_grad_{}_{}.pth'.format(args.checkepoch, args.checkpoint))
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      DA_cst_loss,tgt_DA_cst_loss,GA,GB=\
          fasterRCNN(im_data, im_info, gt_boxes, num_boxes,need_backprop,
                     tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes, tgt_need_backprop,lr)



      loss = (1-GA)*(rpn_loss_cls.mean() + rpn_loss_box.mean() \
           + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean())\
             +(1-GB)*args.lamda*(DA_img_loss_cls.mean()+DA_ins_loss_cls.mean()+tgt_DA_img_loss_cls.mean()+tgt_DA_ins_loss_cls.mean()+DA_cst_loss.mean()+tgt_DA_cst_loss.mean())
      loss_temp += loss.item()
      ...
      if step % args.disp_interval == 0:
        ...
        print("[session %d][epoch %2d][iter %4d/%4d] 梯度调和loss: %.4f, lr: %.2e" \
    ...
    if epoch==args.max_epochs or (epoch % 1 == 0):
        save_name = os.path.join(output_dir, '20211019_city2fogcity_grad_%d_%d.pth' % (args.session, epoch))
        save_checkpoint({
