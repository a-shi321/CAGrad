...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
        ...
        self.opt_g = optim.SGD(filter(lambda p: p.requires_grad,self.RCNN_base.parameters()),lr=lr)
        ...
        loss_det=rpn_loss_cls.mean() + rpn_loss_bbox.mean()+ RCNN_loss_cls.mean() + RCNN_loss_bbox.mean()
        loss_da=DA_img_loss_cls.mean()+DA_ins_loss_cls.mean()+tgt_DA_img_loss_cls.mean()+tgt_DA_ins_loss_cls.mean()+DA_cst_loss.mean()+tgt_DA_cst_loss.mean()

        loss_det.backward(retain_graph=True)
        grad1 = self.getgrad(self.RCNN_base)
        self.opt_g.zero_grad()

        loss_da.backward(retain_graph=True)
        grad2 = self.getgrad(self.RCNN_base)
        self.opt_g.zero_grad()
        ...
        return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
               DA_img_loss_cls,DA_ins_loss_cls,tgt_DA_img_loss_cls,tgt_DA_ins_loss_cls,DA_cst_loss,tgt_DA_cst_loss,GA,GB
